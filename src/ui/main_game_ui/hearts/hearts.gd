...
func draw_hearts():
	...
	var heart_yellow_num: float = (
		heart_yellow_max if heart_num > heart_yellow_max * 2 else heart_num / 2
	)
	...
	var heart_red: Array[Sprite2D] = get_heart(heart_red_num, heart_fill_red, delta_red)
	var heart_yellow: Array[Sprite2D] = get_heart(heart_yellow_num, heart_fill_yellow, delta_yellow)
